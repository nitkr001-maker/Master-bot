...
# ==========================================
TELEGRAM_BOT_TOKEN = "8699544292:AAFoAxQcZD7_-I5YVmTSqwd8LE4BE1MTqM0"
ADMIN_CONTACT = "@ntn_00"
ADMIN_ID = 5644151841  
...
def save_sent_poll(channel_id, message_id, session_id="default", session_label="Unnamed Quiz Session"):
    """Saves sent polls grouped by Session ID for targeted deletion later."""
    data = {}
...
def parse_csv_content(csv_content: str):
    ...
    for i, row in enumerate(reader):
        ...
        try:
            # Smart answer parsing (safely extracts last character if format is weird)
            raw_ans = row[5].strip().upper()
            clean_ans = raw_ans[-1] if raw_ans else 'A'
            
            # Optional 7th Column for Explanations
...
async def get_channel_selection_markup(context: ContextTypes.DEFAULT_TYPE, next_action: str) -> InlineKeyboardMarkup:
    """Generates an inline keyboard for selecting multiple target channels."""
...
async def send_polls_task(context: ContextTypes.DEFAULT_TYPE, admin_chat_id: int, questions: list, target_channels: list):
    """The central task responsible for safely sending polls"""
    ...
    for i, q in enumerate(questions):
        ...
        await asyncio.sleep(2) # Protect against flooding multiple channels
...
async def post_init(application: Application) -> None:
    ...
    print("✅ Menu Commands configured successfully on Telegram!")
...
async def unauth_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    else: await update.message.reply_text("⚠️ User is not in the authorized list.")
...
async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.user_data.get('poll_questions',[]): return await update.message.reply_text("❌ No questions loaded. Use /uploadcsv first.")
    if not context.user_data.get('channels',[]): return await update.message.reply_text("❌ No channels saved. Use /setchannel first.")
...
async def clearcsv(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data['poll_questions'] =[]
...
async def deletequizzes_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        with open(TRACKING_FILE, "r") as f: data = json.load(f)
        tracked_channels = list(data.keys())
    except Exception: tracked_channels =[]

    if not tracked_channels:
        return await update.message.reply_text("❌ No target channels found with saved quizzes in my memory.")

    keyboard = [[InlineKeyboardButton(f"🗑 {ch}", callback_data=f"delq_ch_{ch}")] for ch in tracked_channels]
    await update.message.reply_text("🗑 **Select which channel you want to clean up:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("neg_"):
        ...
        questions = context.user_data.get('poll_questions',[])
        if not questions:
            return await query.edit_message_text("❌ No questions loaded. Use /uploadcsv first.")
    ...
    if data.startswith("help_"):
        ...
        elif data == "help_uploadcsv": guide_info = "📂 **Upload CSV / Text**\nUse `/uploadcsv`, choose file or text upload. You can stack multiple files to combine them! Click *Done Uploading* when finished."
        elif data == "help_setchannel": guide_info = "📢 **Multi-Channel Guide**\nYou can post to multiple groups at once!\n\nUse `/setchannel @MyChannel -10012345` to add multiple channels simultaneously. Then type `/channels` to see them."
        elif data == "help_deletequizzes": guide_info = "🗑 **Delete Session Guide**\nUse `/deletequizzes`. The bot dynamically remembers every channel it posts to. Select a channel, then choose exactly which Quiz Session (by Topic/Count) you want to securely remove."
    ...
    if data.startswith("togglech_"):
        ...
        if next_action == "start_quiz":
            ...
            # 🚀 Added Immediately Button!
            keyboard = [[InlineKeyboardButton("🚀 Immediately", callback_data="sched_0")],[InlineKeyboardButton("5 Mins", callback_data="sched_5"), InlineKeyboardButton("15 Mins", callback_data="sched_15")],[InlineKeyboardButton("30 Mins", callback_data="sched_30"), InlineKeyboardButton("1 Hour", callback_data="sched_60")],
    ...
    if data.startswith("sched_") and data != "sched_manual":
        ...
        if mins == 0:
            return await query.edit_message_text(f"🚀 Quiz posting started IMMEDIATELY across `{len(selected)}` channels.", parse_mode="Markdown")
    ...
    if data.startswith("schedp_"):
        ...
        if mins == 0:
            return await query.edit_message_text(f"🚀 Custom post sent IMMEDIATELY to `{len(selected)}` channels.", parse_mode="Markdown")
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == 'wait_channel_id':
        if update.message.text:
            new_chs = update.message.text.split()
            saved_channels = context.user_data.setdefault('channels',[])
            added =[]
        ...
        if update.message.text:
            ...
            if not questions:
                ...
                existing = context.user_data.get('poll_questions',[])
                existing.extend(questions)
                context.user_data['poll_questions'] = existing
                # ✅ Fixed to allow stacking!
                keyboard = [[InlineKeyboardButton("✅ Done Uploading", callback_data="cancel_action")]]
                await update.message.reply_text(f"✅ Successfully loaded `{len(questions)}` questions! (Total loaded: {len(existing)})\n\n📂 **You can paste another text to stack more questions**, or click 'Done'.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        else:
        ...
        if update.message.document:
            try:
                ...
                if not questions:
                    ...
                    # ✅ Fixed to allow stacking!
                    keyboard = [[InlineKeyboardButton("✅ Done Uploading", callback_data="cancel_action")]]
                    await update.message.reply_text(f"✅ Successfully loaded `{len(questions)}` questions from file! (Total loaded: {len(existing)})\n\n📂 **You can send another CSV file to stack them**, or click 'Done'.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
