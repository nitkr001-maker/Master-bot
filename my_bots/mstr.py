...
# AI Generation Support via OpenAI (GPT-4o) & PyMuPDF (for scanned PDF to image conversion)
try:
...
# ==========================================
TELEGRAM_BOT_TOKEN = "8689120822:AAF3NMQ9ZAbqomWgBP0aQMf_6c_MOLmfePQ"
ADMIN_CONTACT = "@Mr_outlaw001"
ADMIN_ID = 6915757343  # ⚠️ CHANGE THIS TO YOUR ACTUAL NUMERIC TELEGRAM USER ID

# 🤖 GET YOUR API KEY FROM: https://platform.openai.com/api-keys
OPENAI_API_KEY = "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"
...
async def pdftocsv_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    ai_status = "🟢 ENABLED (ChatGPT Vision GPT-4o)" if AI_SUPPORT and OPENAI_API_KEY != "YOUR_OPENAI_API_KEY_HERE" else "🔴 DISABLED (Will only extract pre-formatted text)"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == 'wait_test_name':
        ...
        try:
            ...
            ai_enabled = AI_SUPPORT and OPENAI_API_KEY and OPENAI_API_KEY != "YOUR_OPENAI_API_KEY_HERE"
            ...
            # --- OPENAI (GPT-4o) PROCESSING ENGINE ---
            if ai_enabled:
                await status_msg.edit_text("🤖 **ChatGPT Vision Active:** Converting scanned PDF pages to high-res images for analysis... This may take a minute.")
                try:
                    ...
                    # To prevent crazy OpenAI token costs, we limit to max 10 pages per run
                    max_pages = min(len(pdf_document), 10)
                    if len(pdf_document) > 10:
                        await status_msg.edit_text("⚠️ **Large Book Detected.** Processing the first 10 pages to prevent OpenAI Rate Limits/Costs...")
                    
                    for page_num in range(max_pages):
                        page = pdf_document.load_page(page_num)
                        pix = page.get_pixmap(dpi=150) # Keep resolution optimal for GPT-4o
                    ...
                    prompt = """
ROLE:
Act as a strict Assessment Generator. Your goal is to create Multiple Choice Questions (MCQs) based only on the provided text/images.
...
"""
                    ...
                    # Construct message array with all images
                    ...
                    await status_msg.edit_text("🧠 **ChatGPT Processing:** Analyzing images and generating Flawless Bilingual CSV...")
                    ...
                    await status_msg.edit_text("⚠️ OpenAI API Error. Falling back to basic structural extraction...")
            ...
            if count == 0:
                ...
                if not text.strip(): 
                    return await status_msg.edit_text("❌ The PDF appears to be empty or contains only images.\n\n⚠️ **SOLUTION:** You need to enable the **ChatGPT Vision Engine** to read image-based PDFs. Open the python script, set your `OPENAI_API_KEY`, and restart the bot!")
            ...
            await status_msg.edit_text(f"❌ An error occurred while reading or parsing the PDF file: {e}")
...
def main() -> None:
    ...
    print("✅ Advance ChatGPT MCQ Bot is Running securely!")
